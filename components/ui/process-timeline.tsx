...
export const ProcessCardTitle = React.forwardRef<
  ...
  React.HTMLAttributes<HTMLDivElement>
>(({ className, ...props }, ref) => (
  <div ref={ref} className={cn("p-6 shrink-0", className)} {...props} />
))
...
export const ProcessCard: React.FC<ProcessCardProps> = ({
  ...
  const x = useTransform(scrollYProgress, (p) => {
    const currentInnerWidth = typeof window !== 'undefined' ? window.innerWidth : 1000;
    const final_x = -((width ?? 0) * index) + 80 * index;
    if (p <= start) return currentInnerWidth;
    if (p >= end) return final_x;
    
    const ratio = (p - start) / (end - start);
    return currentInnerWidth + ratio * (final_x - currentInnerWidth);
  });
  return (
    <motion.div
      ref={ref}
