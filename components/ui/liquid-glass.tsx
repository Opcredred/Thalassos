  ...
  >
    <div className="flex items-center justify-center gap-2 rounded-3xl p-3 py-0 px-0.5 overflow-hidden">
      {icons.map((icon, index) => (
        <div
          key={index}
          className="w-12 h-12 sm:w-16 sm:h-16 flex items-center justify-center transition-all duration-700 hover:scale-110 cursor-pointer"
          style={{
        ...
        >
          {icon.src ? (
            ...
            <div className="w-full h-full flex items-center justify-center">{icon.icon}</div>
          ) : null}
        </div>
      ))}
    </div>
